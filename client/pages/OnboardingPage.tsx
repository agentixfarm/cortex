/**
 * OnboardingPage - 4-step first-time user wizard.
 *
 * Steps:
 *  1. Welcome - branding and value prop
 *  2. Select Folders - pick folders to watch
 *  3. Scanning Progress - live indexing feedback
 *  4. Spaces Ready - discovered spaces and CTA
 */
...
export default function OnboardingPage() {
  ...
  const addCustomFolder = useCallback(async () => {
    if (isTauri()) {
      try {
        ...
        // Dialog plugin not available, fall through to text input
      }
    }
    // In browser mode, use the text input
  ...
  useEffect(() => {
    if (step === 2 && indexingStore.isIndexing) {
      setScanningProgress({
